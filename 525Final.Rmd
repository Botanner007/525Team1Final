...
We first attempt to fit a model with no transformation to the variable
...
anova(CreditM1)
```
We get some pretty consistent variance, but we also have a problem where a balance of 0 is negativly affecting out plots. We will test some transformations to the Balance to see if that can solve our problem.
...
anova(CreditM4)
```
These transformations are also effected by the presence of 0's in our data. For the purpose of this assignment, we feel that omitting 0's will not effect our conclusion, but allow us to focus on the more important part of the dataset. We will continue with our search for a fitted model using a filter to exclude 0's, as data set Credit2. 
...
Our initial models will contain all variables present in the subsetted data set as predictors, except Balance, which will be the response variable. In order to find the best fit, we will use every transfromation that we think could help fit the model. 
...
```{r}
##This is the log model comparisons
CR2Mod2<-lm(log(Balance)~Income+Limit+Rating+Cards+Age+Student, data=Credit2)
summary(CRMod2)
par(mfrow=c(2,2))
plot(CR2Mod2)
anova(CR2Mod2)
anova(CR2Mod2,CR2Mod1)
```


```{r}
##This is the no transformation model comparisons
...
```{r}
##This is the log model comparisons
CR2Mod3<-lm(log(Balance)~Income+Limit+Rating+Cards+Age+Student+Income*Limit+Rating*Limit, data=Credit2)
summary(CR2Mod3)
par(mfrow=c(2,2))
plot(CR2Mod3)
anova(CR2Mod3)
anova(CR2Mod2,CR2Mod3)
```
...
```{r}
##This is the log model comparisons
CR2Mod4<-lm(log(Balance)~Income+Limit+Rating+Age+Student+Income*Limit+Rating*Limit+Income*Rating*Limit*Age, data=Credit2)
summary(CR2Mod4)
par(mfrow=c(2,2))
plot(CR2Mod4)
anova(CR2Mod4)
anova(CR2Mod3,CR2Mod4)
```
This Model a few other interactions that may have an effect. (I am just exploring here)

Maybe we need a BoxCox to help normalize our data? 
